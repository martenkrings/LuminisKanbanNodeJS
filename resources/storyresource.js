                        ...
                        Role.findOne({_id: relation.roleId}, function (roleErr, role) {
                            ...
                            if (role.manageStories) {
                                ...
                                newStory.save(function (storyErr) {
                                    if (storyErr) {
                                        return res.status(500).json({error: storyErr.message});
                                    }
...
router.post('/movestory', function (req, res) {
    ...
    jwt.verify(token, req.app.get('private-key'), function (err, decoded) {
        ...
        if (err) {
            return res.status(401).json({error: 'Forbidden, bad token.'});
        }
                    ...
                    RoleToUser.findOne({userId: user._id, boardId: board._id}, function (relationErr, relation) {
                        ...
                        Role.findOne({_id: relation.roleId}, function (roleErr, role) {
                            ...
                            Story.findOne({_id: req.body.storyId}, function(storyErr, story) {
                                ...
                                for(var i = 0; i < role.moveFrom.length; i ++) {

                                    if (role.moveFrom[i].columnId == req.body.columnId) {

                                        Columns.findOne({boardId: board._id, position: column.position + 1}, function(columnsErr, columnTo) {
